...
//                    val insertSchema = mutationRequest.insert_schema.find { schema -> schema.collection == op.collection }
//                    requireNotNull(insertSchema) { "No insert schema found for table ${op.collection}" }
//
...
//
////                    if (!hasAutoIncPK) {
//                        println("Table ${op.collection} has no auto-increment PK, using temp table")
...
//                }
//
//                is MutationOperation.ProcedureMutationOperation -> throw Exception("Unsupported")
//            }
//        }
//    }
//
...
//        val insertSchema = mutation.insert_schema.find { schema -> schema.collection == op.collection }
//        requireNotNull(insertSchema) { "No insert schema found for table ${op.collection}" }
//
...
//            val tempTableDefinition = DSL.select().from(DSL.name(op.collection)).limit(0)
...
//            val insertIntoTargetFromTemp = DSL.insertInto(DSL.table(DSL.name(op.collection)))
...
//                                relationships = mutation.collection_relationships
//                            )
//                        } ?: DSL.trueCondition()
//                    )
//                )
